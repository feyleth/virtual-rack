...
export default (props: Props) => {
  ...
  return (
    <div class={styles.node} oncontextmenu={(e) => show(e)}>
      ...
      <div>
        <For each={Array.from(props.node.ports).filter((el) => el.direction === Direction.In).sort()}>
          {(port) => (
      ...
      <div class={styles.out}>
        ...
        <For each={Array.from(props.node.ports).filter((el) => el.direction === Direction.Out).sort()}>
          {(port) => (
