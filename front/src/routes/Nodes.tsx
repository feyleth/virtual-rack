...
import { Node as NodeType } from "../api/node.ts";
import Link, { LinkType } from "../components/Link.tsx";

function sortNode(nodes: NodeType[]): NodeType[] {

  let return_nodes = nodes.map(node => ({ ...node, ports: node.ports.sort((a, b) => a.name.localeCompare(b.name)) }))

  return return_nodes.sort((a, b) => a.name.localeCompare(b.name));
}
...
export default () => {
  ...
  events.addEventListener("init state", (e) => {
    ...
    setState({ ...state, nodes: sortNode(state.nodes) })
  })
